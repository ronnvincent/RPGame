...
description: Build or extend RPGs with the RPGJS framework (@rpgjs/client, @rpgjs/server, npx degit rpgjs/starter) — GUI system (HUD, dialog, menus, inventories, sprite-attached tooltips) via CanvasEngine .ce or Vue overlays, prebuilt shops, loot/item drops, instanced dungeons, multiplayer parties, and character skills/abilities (@Skill entries, learnSkill/useSkill, @rpgjs/action-battle combat with BattleAi) — including a skill's animation/SFX not matching what it does, or missing sound, across a roster. ALSO covers co-op multiplayer desync/sync bugs — one player's client shows a different game state than another's, like skill effects/animations that only some players see, monsters or enemies invisible to one player but hittable by another, a wave/enemy-spawn system whose count or progress disagrees between players, a dungeon run that completes or returns to town for one player while the rest of the party stays stuck inside, or any "works fine until we're actually in the dungeon" / host-vs-joining-player inconsistency. Use whenever the user mentions RPGJS, RPG-JS, "rpgjs", @rpgjs packages, or is working in an RPGJS project (client.ts/server.ts/modules, RpgPlayer, RpgMap, RpgClientEngine, RpgWorld) — including "dungeon", "shop", "loot", "party", "co-op", "multiplayer", skill/ability VFX, combat sfx, an attack/spell with the wrong animation or no sound, or players out of sync / seeing different things / hindi nagkakasabay / hindi sync sa isa't isa. Do NOT use the plain-canvas pixel-game approach for RPGJS — it has its own architecture, GUI, and terminology instead of hand-rolled canvas code.
---
...
**skill abilities — animation & SFX wiring** · **debugging co-op desync (host vs. joining
player see different game states)** · v4 legacy notes · common pitfalls.
...
asymmetric by direction, don't go back to the animation/SFX wiring itself (see "Skill abilities" above
— that's for the wrong animation playing, not this). Instead grep the skill's `onUse`/effect code for
anything that branches on who's the host:
```ts
