...
const A = { uuid: `uuid-rh-${RUN}`, name: 'HostA', shortId: `RH${RUN}`.toUpperCase() };
const B = { uuid: `uuid-rg-${RUN}`, name: 'GuestB', shortId: `RG${RUN}`.toUpperCase() };
...
const waitFor = (sock, event, ms = T(2000)) => new Promise(resolve => {
...
const run = async () => {
  ...
  const bSees1 = waitFor(b, 'enemy_sync', T(1200));
  ...
  const bNewLobby = waitFor(b, 'lobby_update', T(1200));
  const bError = waitFor(b, 'lobby_error', T(1200));
