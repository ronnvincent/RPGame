...
export type PlayerBuffStat =
  ...
  | 'airMobility'
  | 'deathPrevention';
export type EnemyStatusKind = 'slow' | 'poison' | 'burn' | 'stun' | 'frailty' | 'taunt';
...
export const SKILL_IDENTITY_MATRIX: Record<SkillId, SkillIdentityEntry> = {
  ...
  w_3: { description: 'Shield-bash a short cone for 110% physical damage, strong stagger, and a 1.2 second stun.', mechanics: { delivery: { kind: 'melee', shape: 'cone', origin: 'aim' }, hits: hits(), payload: direct({ knockback: 1.8, statuses: [{ kind: 'stun', duration: 1.2 }] }) }, visual: visual('w_3', 'warrior', 5, 'shield-shock-ring', 'hard-forward-impact', 'instant', 'stun') },
  ...
  w_6: { description: 'Leap into a telegraphed Earth Shatter, dealing 570% physical damage with a heavy knock-up on the cinematic impact.', mechanics: { delivery: { kind: 'area', shape: 'radial', origin: 'ground' }, hits: hits(), payload: direct({ knockUp: 5 }), movement: { kind: 'leap', distance: 150, delayMs: 360, knockUp: 9 } }, visual: visual('w_6', 'warrior', 4, 'golden-earth-fissure', 'leap-crack-rock-burst', 'ultimate-impact') },
  ...
  m_2: { description: 'Release a self-centered frost nova for 130% ice damage and a real 50% slow for 4 seconds.', mechanics: { delivery: { kind: 'area', shape: 'radial', origin: 'caster' }, hits: hits(), payload: direct({ statuses: [{ kind: 'slow', duration: 4, magnitude: 0.5 }] }) }, visual: visual('m_2', 'mage', 2, 'ice-ring-and-shards', 'outward-freeze', 'instant', 'slow') },
  m_3: { description: 'Chain 180% total lightning damage through up to 4 distinct targets with decreasing jump strength.', mechanics: { delivery: { kind: 'chain', origin: 'nearest', maxTargets: 4 }, hits: { count: 4, targeting: 'distinct', intervalMs: 80, falloff: [1, 0.8, 0.65, 0.5] }, payload: direct() }, visual: visual('m_3', 'mage', 2, 'connected-lightning-arcs', 'ordered-target-jumps', 'sequence') },
  ...
  m_5: { description: 'Create a 5-second blizzard zone dealing 220% total ice damage over 10 ticks and slowing enemies by 40%.', mechanics: { delivery: { kind: 'zone', shape: 'radial', origin: 'aim', duration: 5, tickInterval: 0.5, zoneType: 'blizzard' }, hits: hits(10, 'same-area', 500), payload: direct({ statuses: [{ kind: 'slow', duration: 1.2, magnitude: 0.4 }] }) }, visual: visual('m_5', 'mage', 2, 'confined-hail-zone', 'falling-shards-and-mist', 'zone-tick', 'slow') },
  m_6: { description: 'Mark an area, then land 4 staggered meteors for 780% total fire damage on the ultimate impact sequence.', mechanics: { delivery: { kind: 'distributed', shape: 'radial', origin: 'aim', maxTargets: 4 }, hits: hits(4, 'rain', 140), payload: direct({ directDamageShare: 0.92, statuses: [{ kind: 'burn', duration: 4, tickInterval: 1, damageShare: 0.08 }] }) }, visual: visual('m_6', 'mage', 7, 'descending-meteor-rocks', 'warning-circles-then-impacts', 'ultimate-impact', 'burn') },
  ...
  p_6: { description: 'Drop a celestial hammer for 600% holy damage, knockdown, and a 1.5 second stun on the cinematic impact.', mechanics: { delivery: { kind: 'area', shape: 'radial', origin: 'aim' }, hits: hits(), payload: direct({ knockUp: 4, statuses: [{ kind: 'stun', duration: 1.5 }] }) }, visual: visual('p_6', 'paladin', 0, 'descending-celestial-hammer', 'pillar-crush-and-starburst', 'ultimate-impact', 'stun') },
  ...
  ar_4: { description: 'Fire a burning arrow that explodes on impact for 200% fire damage in an 80px area.', mechanics: { delivery: { kind: 'projectile', origin: 'aim', projectile: 'arrow', speed: 13 }, hits: hits(), payload: direct({ knockback: 1 }) }, visual: visual('ar_4', 'archer', 7, 'burning-arrowhead', 'arrow-flight-then-explosion', 'projectile') },
  ...
  n_2: { description: 'Tether the nearest enemy for 160% dark damage and heal for 50% of damage actually dealt.', mechanics: { delivery: { kind: 'targeted', origin: 'nearest', maxTargets: 1 }, hits: hits(), payload: direct({ lifesteal: 0.5 }) }, visual: visual('n_2', 'necromancer', 1, 'crimson-violet-tether', 'target-to-caster-return', 'instant') },
  ...
  n_4: { description: 'Hex nearby enemies, reducing their defense by 40% for 7 seconds without dealing direct damage.', mechanics: { delivery: { kind: 'area', shape: 'radial', origin: 'caster' }, hits: hits(), payload: utility({ statuses: [{ kind: 'frailty', duration: 7, magnitude: 0.4 }] }) }, visual: visual('n_4', 'necromancer', 1, 'overhead-curse-sigils', 'purple-decay-pulse', 'instant', 'frailty'), damageMultiplier: 0 },
  ...
  b_3: { description: 'Leap to the target point and land for 190% physical area damage with heavy knock-up.', mechanics: { delivery: { kind: 'area', shape: 'radial', origin: 'aim' }, hits: hits(), payload: direct({ knockUp: 5 }), movement: { kind: 'leap', distance: 240, delayMs: 260, knockUp: 13 } }, visual: visual('b_3', 'berserker', 7, 'red-leap-crater', 'trail-dust-rock-impact', 'sequence') },
  ...
  pr_1: { description: 'Fire a thin cyan-gold holy bolt for 75% damage; it never enters a melee combo.', mechanics: { delivery: { kind: 'projectile', origin: 'aim', projectile: 'energy_ball', speed: 12 }, hits: hits(), payload: direct() }, visual: visual('pr_1', 'priest', 2, 'thin-celestial-ray', 'clean-light-impact', 'projectile') },
  ...
  pr_5: { description: 'Send a broad holy wall forward for 230% damage with strong pushback.', mechanics: { delivery: { kind: 'area', shape: 'wall', origin: 'aim' }, hits: hits(), payload: direct({ knockback: 2.2 }) }, visual: visual('pr_5', 'priest', 2, 'broad-cyan-gold-wall', 'single-expanding-wave', 'instant') },
  ...
  ni_5: { description: 'Teleport through up to 4 distinct targets for 250% combined lightning damage.', mechanics: { delivery: { kind: 'distributed', origin: 'nearest', maxTargets: 4 }, hits: hits(4, 'distinct', 75), payload: direct({ forceCrit: true }) }, visual: visual('ni_5', 'ninja', 2, 'four-lightning-paths', 'individually-timed-teleports', 'sequence') },
  ni_6: { description: 'Freeze the scene, then distribute 12 dimensional slashes for 740% total dark damage on the ultimate impact sequence.', mechanics: { delivery: { kind: 'distributed', origin: 'nearest', maxTargets: 12 }, hits: hits(12, 'distributed', 38), payload: direct({ forceCrit: true }) }, visual: visual('ni_6', 'ninja', 3, 'emerald-dimensional-cuts', 'time-stop-twelve-slashes', 'ultimate-impact') },
  ...
  d_2: { description: 'Rise into the air, mark the landing, then plunge for 220% physical area damage and knock-up.', mechanics: { delivery: { kind: 'area', shape: 'radial', origin: 'aim' }, hits: hits(), payload: direct({ knockUp: 5 }), movement: { kind: 'leap', distance: 220, delayMs: 250, knockUp: 13 } }, visual: visual('d_2', 'dragoon', 2, 'wing-gust-and-spear-marker', 'rise-mark-plunge-crater', 'sequence') },
  ...
  d_6: { description: 'Summon an elder-dragon avatar whose 6 beam ticks deal 760% total fire damage on the ultimate impact sequence.', mechanics: { delivery: { kind: 'distributed', shape: 'line', origin: 'caster', maxTargets: 6 }, hits: hits(6, 'forward', 100), payload: direct({ directDamageShare: 0.92, statuses: [{ kind: 'burn', duration: 4, tickInterval: 1, damageShare: 0.08 }] }), summon: { kind: 'dragon_avatar', count: 1, duration: 2.2, damageScale: 1 } }, visual: visual('d_6', 'dragoon', 7, 'elder-dragon-fire-beam', 'entrance-sweep-ground-fire', 'ultimate-impact', 'burn') },
};
